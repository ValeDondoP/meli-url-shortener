...
class URLShortenerService:
    ...
    def create_short_url(self, original_url: str) -> str:
        ...
        if not self.get_data_by_hash(url_hash=url_hash):
            try:
                ...
                RuntimeError(f"Error al insertar el documento en la base de datos: {e}")

            document = self.db_repository.get_document_by_key(key=url_hash)

            if not document:
                raise RuntimeError(f"No se pudo obtener el documento insertado con url_hash: {url_hash}")
    ...
    def increment_visit_count(self, url_hash: str) -> bool:
        is_updated = self.db_repository.update_document(
            key=url_hash,
            update_fields={"visit_count": 1},
            increment=True
        )

        if is_updated:
            if self.cache_repository.get_data_by_hash(key=url_hash):
                self.cache_repository.increment_field(key=url_hash, field="visit_count")
            return True
        return False
